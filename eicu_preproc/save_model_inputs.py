...
def get_patient_last(patient, data_frame, data_frame_endpoint, max_n_step, mins_dynamic, scales_dynamic):
    ...
    # Extract the last max_n_step time steps, pad if necessary
    num_steps = len(patient_data)
    if num_steps >= max_n_step:
        time_series = patient_data[num_steps - max_n_step: num_steps]
        time_series_endpoint = patient_endpoint[num_steps - max_n_step: num_steps]
    else:
        # Pad with zeros if fewer than max_n_step time steps
        pad_length = max_n_step - num_steps
        time_series = np.pad(patient_data, ((pad_length, 0), (0, 0)), mode='constant', constant_values=0)
        time_series = time_series[:max_n_step]  # Ensure exactly max_n_step
        time_series_endpoint = np.pad(patient_endpoint, ((pad_length, 0), (0, 0)), mode='constant', constant_values=0)
        time_series_endpoint = time_series_endpoint[:max_n_step]
    ...
    time_series_endpoint_all = np.array(time_series_endpoint_all)
