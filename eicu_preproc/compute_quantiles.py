...
def save_variable_quantiles(configs):
    ...
    # Load PIDs as strings
    all_pids_str = mlhc_io.read_list_from_file(configs["included_pid_path"])
    # Convert PIDs to integers
    try:
        all_pids = [int(pid) for pid in all_pids_str]
    except ValueError as e:
        print(f"Error converting PIDs to integers: {e}")
        print(f"Sample PIDs: {all_pids_str[:5]}")
        return
    print(f"Loaded {len(all_pids)} PIDs from {configs['included_pid_path']}")
    print(f"Sample PIDs: {all_pids[:5]}")
    ...
    print(df_lab.patientunitstayid)
    print(all_pids)
