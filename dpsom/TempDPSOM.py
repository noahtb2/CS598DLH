...
def get_data(validation):
    ...
    hf.close()
    data_train, data_val, y_train, endpoints_total_val = train_test_split(
        data_total[:int(len(data_total) * 0.85)],
...
def train_model(model, data_train, data_val, endpoints_total_val, lr_val, prior_val, num_epochs, batch_size, latent_dim,
    ...
    checkpoint_dir = os.path.dirname(modelpath)
    ...
    with tf.Session() as sess:
        ...
        if use_saved_pretrain:
            ...
            for epoch in range(epochs_pretrain):
                ...
                for i in range(num_batches):
                    ...
                    if np.any(np.isnan(batch_data)) or np.any(np.isinf(batch_data)):
                        print(f"Found NaN or Inf in batch_data (pretraining) at epoch {epoch}, batch {i}")
                        return None
                    ...
                    if i % 10 == 0:
                        ...
                        if np.any(np.isnan(batch_val)) or np.any(np.isinf(batch_val)):
                            print(f"Found NaN or Inf in batch_val (pretraining) at epoch {epoch}, batch {i}")
                            return None
                        f_dic = {x: batch_val}
                        f_dic.update(dp)
                        test_loss, summary = sess.run([model.loss_reconstruction_ze, summaries], feed_dict=f_dic)
                        if math.isnan(test_loss):
                            print(f"NaN detected in test_loss (pretraining) at epoch {epoch}, batch {i}")
                            return None
                        ...
                        if math.isnan(train_loss):
                            print(f"NaN detected in train_loss (pretraining) at epoch {epoch}, batch {i}")
                            return None
            ...
            for epoch in range(epochs_pretrain // 3):
                ...
                for i in range(num_batches):
                    batch_data, ii = next(train_gen)
                    if np.any(np.isnan(batch_data)) or np.any(np.isinf(batch_data)):
                        print(f"Found NaN or Inf in batch_data (SOM init 1) at epoch {epoch}, batch {i}")
                        return None
                    ...
                    if i % 10 == 0:
                        batch_val, _, ii = next(val_gen)
                        if np.any(np.isnan(batch_val)) or np.any(np.isinf(batch_val)):
                            print(f"Found NaN or Inf in batch_val (SOM init 1) at epoch {epoch}, batch {i}")
                            return None
                        f_dic = {x: batch_val}
                        f_dic.update(dp)
                        test_loss, summary = sess.run([model.loss_a, summaries], feed_dict=f_dic)
                        if math.isnan(test_loss):
                            print(f"NaN detected in test_loss (SOM init 1) at epoch {epoch}, batch {i}")
                            return None
                        test_writer.add_summary(summary, tf.train.global_step(sess, model.global_step))
                        f_dic = {x: batch_data}
                        f_dic.update(dp)
                        ...
                        if math.isnan(train_loss):
                            print(f"NaN detected in train_loss (SOM init 1) at epoch {epoch}, batch {i}")
                            return None
                        train_writer.add_summary(summary, tf.train.global_step(sess, model.global_step))
                    pbar.set_postfix(epoch=epoch, train_loss=train_loss, test_loss=test_loss, refresh=False)
                    pbar.update(1)
            ...
            for epoch in range(epochs_pretrain // 3):
                ...
                for i in range(num_batches):
                    batch_data, ii = next(train_gen)
                    if np.any(np.isnan(batch_data)) or np.any(np.isinf(batch_data)):
                        print(f"Found NaN or Inf in batch_data (SOM init 2) at epoch {epoch}, batch {i}")
                        return None
                    ...
                    if i % 10 == 0:
                        batch_val, _, ii = next(val_gen)
                        if np.any(np.isnan(batch_val)) or np.any(np.isinf(batch_val)):
                            print(f"Found NaN or Inf in batch_val (SOM init 2) at epoch {epoch}, batch {i}")
                            return None
                        f_dic = {x: batch_val}
                        f_dic.update(dp)
                        test_loss, summary = sess.run([model.loss_a, summaries], feed_dict=f_dic)
                        if math.isnan(test_loss):
                            print(f"NaN detected in test_loss (SOM init 2) at epoch {epoch}, batch {i}")
                            return None
                        test_writer.add_summary(summary, tf.train.global_step(sess, model.global_step))
                        f_dic = {x: batch_data}
                        f_dic.update(dp)
                        train_loss, summary = sess.run([model.loss_a, summaries], feed_dict=f_dic)
                        if math.isnan(train_loss):
                            print(f"NaN detected in train_loss (SOM init 2) at epoch {epoch}, batch {i}")
                            return None
                        train_writer.add_summary(summary, tf.train.global_step(sess, model.global_step))
                    pbar.set_postfix(epoch=epoch, train_loss=train_loss, test_loss=test_loss, refresh=False)
                    pbar.update(1)
            ...
            for epoch in range(epochs_pretrain // 3):
                ...
                for i in range(num_batches):
                    batch_data, ii = next(train_gen)
                    if np.any(np.isnan(batch_data)) or np.any(np.isinf(batch_data)):
                        print(f"Found NaN or Inf in batch_data (SOM init 3) at epoch {epoch}, batch {i}")
                        return None
                    f_dic = {x: batch_data, lr_val: 0.01}
                    f_dic.update(dp)
                    train_step_som.run(feed_dict=f_dic)
                    if i % 10 == 0:
                        batch_val, _, ii = next(val_gen)
                        if np.any(np.isnan(batch_val)) or np.any(np.isinf(batch_val)):
                            print(f"Found NaN or Inf in batch_val (SOM init 3) at epoch {epoch}, batch {i}")
                            return None
                        f_dic = {x: batch_val}
                        f_dic.update(dp)
                        test_loss, summary = sess.run([model.loss_a, summaries], feed_dict=f_dic)
                        if math.isnan(test_loss):
                            print(f"NaN detected in test_loss (SOM init 3) at epoch {epoch}, batch {i}")
                            return None
                        test_writer.add_summary(summary, tf.train.global_step(sess, model.global_step))
                        f_dic = {x: batch_data}
                        f_dic.update(dp)
                        train_loss, summary = sess.run([model.loss_a, summaries], feed_dict=f_dic)
                        if math.isnan(train_loss):
                            print(f"NaN detected in train_loss (SOM init 3) at epoch {epoch}, batch {i}")
                            return None
                        train_writer.add_summary(summary, tf.train.global_step(sess, model.global_step))
                    pbar.set_postfix(epoch=epoch, train_loss=train_loss, test_loss=test_loss, refresh=False)
                    pbar.update(1)
        ...
        for epoch in range(num_epochs):
            ...
            q = []
            for t in range(19):
                q_t = sess.run(model.q, feed_dict={
                    x: data_train[int(len(data_train) / 20) * t: int(len(data_train) / 20) * (t + 1)]})
                if np.any(np.isnan(q_t)) or np.any(np.isinf(q_t)):
                    print(f"NaN or Inf detected in model.q (train) at t={t}")
                    return None
                q.extend(q_t)
            q_last = sess.run(model.q, feed_dict={x: data_train[int(len(data_train) / 20) * 19:]})
            if np.any(np.isnan(q_last)) or np.any(np.isinf(q_last)):
                print("NaN or Inf detected in model.q (train, last segment)")
                return None
            q.extend(q_last)
            q = np.array(q)
            print(f"q (train) shape: {q.shape}, q min/max: {q.min()}/{q.max()}")
            ppt = model.target_distribution(q)
            if np.any(np.isnan(ppt)) or np.any(np.isinf(ppt)):
                print("NaN or Inf detected in ppt")
                return None
            print(f"ppt shape: {ppt.shape}, ppt min/max: {ppt.min()}/{ppt.max()}")
            ...
            for t in range(9):
                ...
                if np.any(np.isnan(q_t)) or np.any(np.isinf(q_t)):
                    print(f"NaN or Inf detected in model.q (val) at t={t}")
                    return None
                q.extend(q_t)
            q_last = sess.run(model.q, feed_dict={x: data_val[int(len(data_val) / 10) * 9:]})
            if np.any(np.isnan(q_last)) or np.any(np.isinf(q_last)):
                print("NaN or Inf detected in model.q (val, last segment)")
                return None
            q.extend(q_last)
            q = np.array(q)
            print(f"q (val) shape: {q.shape}, q min/max: {q.min()}/{q.max()}")
            ppv = model.target_distribution(q)
            if np.any(np.isnan(ppv)) or np.any(np.isinf(ppv)):
                print("NaN or Inf detected in ppv")
                return None
            print(f"ppv shape: {ppv.shape}, ppv min/max: {ppv.min()}/{ppv.max()}")

            for i in range(num_batches):
                iterations += 1
                batch_data, ii = next(train_gen)
                if np.any(np.isnan(batch_data)) or np.any(np.isinf(batch_data)):
                    print(f"Found NaN or Inf in batch_data (training) at epoch {epoch}, batch {i}")
                    return None
                ...
                batch_val, _, ii = next(val_gen)
                if np.any(np.isnan(batch_val)) or np.any(np.isinf(batch_val)):
                    print(f"Found NaN or Inf in batch_val (training) at epoch {epoch}, batch {i}")
                    return None
                ...
                if i % 10 == 0:
                    ...
                    if math.isnan(train_loss):
                        print(f"NaN detected in train_loss at epoch {epoch}, batch {i}")
                        loss_components = sess.run([
                            model.loss_reconstruction_ze,
                            model.loss_a,
                            model.loss_prediction
                        ], feed_dict=f_dic)
                        print(f"Reconstruction loss: {loss_components[0]}")
                        print(f"SOM loss: {loss_components[1]}")
                        print(f"Prediction loss: {loss_components[2]}")
                        return None
                    train_writer.add_summary(summary, tf.train.global_step(sess, model.global_step))
                if i % 100 == 0:
        ...
        for epoch in range(50):
            ...
            for i in range(num_batches):
                batch_data, ii = next(train_gen)
                if np.any(np.isnan(batch_data)) or np.any(np.isinf(batch_data)):
                    print(f"Found NaN or Inf in batch_data (finetuning) at epoch {epoch}, batch {i}")
                    return None
                f_dic = {x: batch_data, lr_val: learning_rate, prior_val: prior}
                f_dic.update(dp)
                train_step_prob.run(feed_dict=f_dic)
                if i % 10 == 0:
                    batch_val, _, ii = next(val_gen)
                    if np.any(np.isnan(batch_val)) or np.any(np.isinf(batch_val)):
                        print(f"Found NaN or Inf in batch_val (finetuning) at epoch {epoch}, batch {i}")
                        return None
                    f_dic = {x: batch_val}
                    f_dic.update(dp)
                    test_loss, summary = sess.run([model.loss_prediction, summaries], feed_dict=f_dic)
                    if math.isnan(test_loss):
                        print(f"NaN detected in test_loss (finetuning) at epoch {epoch}, batch {i}")
                        return None
                    test_writer.add_summary(summary, tf.train.global_step(sess, model.global_step))
                    f_dic = {x: batch_data}
                    f_dic.update(dp)
                    ...
                    if math.isnan(train_loss):
                        print(f"NaN detected in train_loss (finetuning) at epoch {epoch}, batch {i}")
                        return None
                    train_writer.add_summary(summary, tf.train.global_step(sess, model.global_step))
                pbar.set_postfix(epoch=epoch, train_loss=train_loss, test_loss=test_loss, refresh=False)
                pbar.update(1)
...
def evaluate_model(model, x, val_gen, len_data_val, modelpath, epochs, batch_size, som_dim, learning_rate, alpha, gamma,
    ...
    num_batches = len_data_val // batch_size  # Use configured batch_size
    ...
    with tf.Session() as sess:
        ...
        for i in range(num_batches):
            ...
            if np.any(np.isnan(batch_data)) or np.any(np.isinf(batch_data)):
                print(f"Found NaN or Inf in batch_data (evaluation) at batch {i}")
                return None
            if np.any(np.isnan(batch_labels)) or np.any(np.isinf(batch_labels)):
                print(f"Found NaN or Inf in batch_labels (evaluation) at batch {i}")
                return None
            f_dic = {x: batch_data}
            f_dic.update(training_dic)
...
def main(input_size, latent_dim, som_dim, learning_rate, decay_factor, alpha, beta, gamma, theta, ex_name, kappa, prior,
    ...
    if not more_runs:
        ...
        if results is None:
            print("Training failed (possibly due to NaN loss). Skipping evaluation.")
            return None

        tf.reset_default_graph()
        val_gen = batch_generator(data_train, data_val, endpoints_total_val, mode="val", batch_size=batch_size)
        ...
        with tf.Session() as sess:
            ...
            if not os.path.exists(modelpath + ".meta"):
                print(f"Checkpoint file {modelpath}.meta does not exist. Training may have failed.")
                return None
            ...
            # Adjust batch size to match state1_o
            prediction_batch_size = num_batches * batch_size  # 1 * 100 = 100
            f_dic = {x: np.zeros((prediction_batch_size, 1, 109)), is_training: False,
                     z_e_p: np.zeros((1 * prediction_batch_size, latent_dim)),
                     z_p: next_z_e_o, init_1: np.zeros((2, batch_size, lstm_dim))}
            x_pred_hat = np.reshape(sess.run(reconstruction, feed_dict=f_dic), (-1, 1, 109))

            for i in range(num_pred - 1):
                print(i)
                inp = data_val[:prediction_batch_size, (t + i), :]
                ...
                f_dic = {x: np.zeros((prediction_batch_size, 1, 109)), is_training: False,
                         z_e_p: np.zeros((max_n_step * prediction_batch_size, latent_dim)),
                         z_p: next_z_e_o, init_1: np.zeros((2, batch_size, lstm_dim))}
                final_x = sess.run(reconstruction, feed_dict=f_dic)
                x_pred_hat = np.concatenate([x_pred_hat, np.reshape(final_x, (-1, 1, 109))], axis=1)

            f_dic = {x: np.zeros((prediction_batch_size, 1, 109)), is_training: False,
                     z_e_p: np.zeros((max_n_step * prediction_batch_size, latent_dim)),
                     z_p: z_e_all[:prediction_batch_size, t - 1, :], init_1: np.zeros((2, batch_size, lstm_dim))}
            final_x = sess.run(reconstruction, feed_dict=f_dic)

        pred_ze = sklearn.metrics.mean_squared_error(np.reshape(next_z_e_o_all[:, :], (-1, latent_dim)),
                                                     np.reshape(z_e_all[:prediction_batch_size, -num_pred:], (-1, latent_dim)))
        # Ensure x_rec and data_val have the same number of samples
        pred_rec = sklearn.metrics.mean_squared_error(np.reshape(x_rec[:prediction_batch_size * max_n_step, :], (-1, 109)),
                                                      np.reshape(data_val[:prediction_batch_size, :], (-1, 109)))
        pred_xhat = sklearn.metrics.mean_squared_error(np.reshape(x_pred_hat, (-1, 109)),
                                                       np.reshape(data_val[:prediction_batch_size, -num_pred:], (-1, 109)))
