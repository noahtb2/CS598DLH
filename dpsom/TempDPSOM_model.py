...
class TDPSOM:
    ...
    def __init__(self, input_size, latent_dim=10, som_dim=[8, 8], learning_rate=1e-4, decay_factor=0.99,
                 decay_steps=2000, input_channels=111, alpha=10., beta=100., gamma=100., kappa=0.,
    ...
    @lazy_scope
    def inputs(self):
        x = tf.placeholder(tf.float32, shape=[None, None, 109], name="x")
        return x

    @lazy_scope
    def x(self):
        x = tf.reshape(self.inputs, [-1, 109])
        return x
